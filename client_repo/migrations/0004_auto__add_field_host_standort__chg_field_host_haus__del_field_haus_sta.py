...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Host.standort'
        db.add_column('client_repo_host', 'standort',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['client_repo.Standort'], null=True, blank=True),
                      keep_default=False)


        # Changing field 'Host.haus'
        db.alter_column('client_repo_host', 'haus_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['client_repo.Haus'], null=True))
        # Deleting field 'Haus.standort'
        db.delete_column('client_repo_haus', 'standort_id')

        # Adding unique constraint on 'Haus', fields ['name']
        db.create_unique('client_repo_haus', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Haus', fields ['name']
        db.delete_unique('client_repo_haus', ['name'])

        # Deleting field 'Host.standort'
        db.delete_column('client_repo_host', 'standort_id')


        # User chose to not deal with backwards NULL issues for 'Host.haus'
        raise RuntimeError("Cannot reverse this migration. 'Host.haus' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'Haus.standort'
        raise RuntimeError("Cannot reverse this migration. 'Haus.standort' and its values cannot be restored.")
